...
const LiveCCTV = {
    ...
    _renderCameraTile(stream) {
        ...
        return `
            <div class="fs-camera-tile ${isTracking ? 'tracking' : ''} ${isOffline ? 'offline' : ''}"
                ...
                <!-- Noise canvas shown as loading state behind the live feed -->
                <canvas class="fs-tile-canvas feed-noise-canvas" id="fs-canvas-${stream.id}" width="640" height="400" style="position:absolute;top:0;left:0;width:100%;height:100%;"></canvas>
                <!-- MJPEG live feed — loads on top of noise canvas -->
                <img
                    src="${feedUrl}"
                    id="fs-feed-${stream.id}"
                    alt="Live feed: ${SentinelHelpers.escapeHtml(stream.name)}"
                    style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;z-index:1;"
                    onerror="this.style.display='none'"
                >
                <div class="fs-tile-placeholder" style="z-index:0;">
            ...
            <div class="fs-tracking-container" id="fs-tracking-container">
                <div class="fs-tracking-grid" data-count="${Math.min(count, 9)}">
                    ${trackingStreams.map(({ stream, data }) => `
                        <div class="fs-tracking-tile" onclick="LiveCCTV.openManualFullscreen('${stream.id}')" style="position:relative;">
                            <canvas class="feed-noise-canvas" id="fs-track-canvas-${stream.id}" width="640" height="400" style="position:absolute;top:0;left:0;width:100%;height:100%;"></canvas>
                            <!-- MJPEG live feed -->
                            <img
                                src="${feedUrl}"
                                id="fs-track-feed-${stream.id}"
    ...
    handleImageSelect(e) {
        ...
        files.forEach(file => {
            if (file.type.startsWith('image/') && file.size <= 10 * 1024 * 1024) {
                ...
                reader.onload = (ev) => {
                    this._targetImages.push({ name: file.name, data: ev.target.result, file: file });
    ...
    startSession() {
        ...
        if (typeof SentinelAPI !== 'undefined') {
            // First, upload targets if any
            const enrollmentPromises = this._targetImages.map(img => {
                const formData = new FormData();
                formData.append('file', img.file);
                formData.append('target_name', this._targetName || img.name);
                return SentinelAPI.enrollTarget(formData)
                    .then(res => console.log('[LiveCCTV] Enrolled target:', res))
                    .catch(err => console.error('[LiveCCTV] Failed to enroll target:', err));
            });
            ...
            Promise.all(enrollmentPromises)
                ...
                .then(res => {
                    ...
                    // Now that backend is ready, navigate and show the dashboard
                ...
                .catch(err => {
                    console.error('[LiveCCTV] Backend pipeline start failed:', err.message);
                    SentinelToast.error('Camera Error', 'Failed to start camera feed. Check RTSP URL.');
                    this._cleanupSession();
                });
        } else {
            // Fallback for simulation mode without backend
            SentinelStore.addActivity({ type: 'system', message: 'Live monitoring session started (SIMULATION)', color: 'green' });
            SentinelToast.success('Session Started', 'Monitoring dashboard activated.');
            SentinelRouter.navigate(location.hash);
            this._initWebSocket();
        }
